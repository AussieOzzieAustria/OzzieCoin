...
const char * const BITCOIN_CONF_FILENAME = "ozziecoin.conf";
const char * const BITCOIN_PID_FILENAME = "ozziecoind.pid";
...
#else
    const char* pszModule = "ozziecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ozziecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ozziecoin
    // Mac: ~/Library/Application Support/Ozziecoin
    // Unix: ~/.ozziecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ozziecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ozziecoin";
#else
    // Unix
    return pathRet / ".ozziecoin";
#endif
#endif
}
